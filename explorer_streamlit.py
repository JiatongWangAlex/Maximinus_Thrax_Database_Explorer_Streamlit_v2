...
def run_standard_search(user_input):
    ...
    try:
        ...
        Sec2_Intervention_Nested_Details AS (
            SELECT 2 AS sg, mt.sequence_id AS seq_id, 1 AS inner_lo, 
                   '* _intervention ' || i.intervention_id || ' :_ ' || 
                   CASE 
...
with st.expander("Close / Open Interactive Leaflet Map Layer Visualizer", expanded=True):
    if st.session_state.get("trigger_map_html"):
        ...
        st.info("No map generated yet. If you have yet to make a search, do so. Then click 'Generate Map' to plot inscriptions matching your query a map.")
# =========================================================
