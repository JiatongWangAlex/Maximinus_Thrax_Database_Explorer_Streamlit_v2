...
def run_standard_search(user_input):
    ...
    try:
        ...
        for rank, ins_id in enumerate(all_matched_ids, 1):
            out_str.append(f"## DOSSIER CARD #{rank} (Database ID: {ins_id})\n")
...
with st.expander("Click to View Site Instructions / Welcome Text", expanded=False, key="welcome_instructions_expander"):
    ...
    ### Keyword Search
    * Use the top bar to search for inscriptions using a keyword or phrase. The report printed using the word or phrase search is very minimalist by design.

    ### Full Inscription Reports:
    * Includes further metadata and a detailed object biography documenting all inscriptions on the object, and all interventions to the each inscription, and the target, extent and method of each intervention. Please generate the full report by copying the inscription ID into the corresponding field and pressing Enter.
    ...
    * Press the Show on Map button after your search or your person / inscription report generation to see all relevant inscriptions on an interactive map.
