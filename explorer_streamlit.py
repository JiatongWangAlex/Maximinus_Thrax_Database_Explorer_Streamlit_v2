...
# ==============================================================================
# STANDALONE PERFORMANCE DEBUGGER BLOCK (Paste at the very bottom of your app)
# ==============================================================================
if "active_inscription_ids" in st.session_state and st.session_state.active_inscription_ids:
    st.markdown("---")
    st.markdown("### ⚡ Live Page Performance Breakdown")
    
    import time
    
    try:
        # 1. Measure database connection speed
        t0 = time.time()
        debug_conn = get_db_connection()
        debug_cursor = debug_conn.cursor()
        t_conn = time.time() - t0
        
        # 2. Measure raw batch extraction performance
        ids_to_test = st.session_state.active_inscription_ids
        
        t1 = time.time()
        debug_reports = get_inscription_report(debug_cursor, ids_to_test)
        t_db_extract = time.time() - t1
        
        debug_conn.close()
        
        # 3. Calculate text payload size
        if isinstance(debug_reports, dict):
            total_chars = sum(len(v) for v in debug_reports.values())
        else:
            total_chars = len(debug_reports)
            
        # 4. Draw metrics cards directly onto the webpage
        col1, col2, col3 = st.columns(3)
        col1.metric("Database Query Speed", f"{t_db_extract:.4f}s")
        col2.metric("DB Connection Open", f"{t_conn:.4f}s")
        col3.metric("Total Markdown Text Size", f"{total_chars:,} chars")
            
        # 5. Print dynamic, on-screen diagnosis based on the results
        if t_db_extract > 0.15:
            st.warning("⚠️ **What's making it slow:** The database queries are still taking too long. Double-check that your SQLite table indices were successfully saved.")
        elif total_chars > 60000:
            st.info("💡 **What's making it slow:** The database is instant, but the text size is massive! Streamlit is freezing your web browser trying to render thousands of lines of text all at once. Try using collapsible expander boxes.")
        else:
            st.success("✅ **What's making it slow:** Both your database and text volume are running optimally right now. If the page still feels delayed, Streamlit is likely executing your entire script file from top to bottom on every click. We can easily fix that with data caching.")
            
    except Exception as debug_err:
        st.error(f"Debugger failed to execute: {debug_err}")
# ==============================================================================
